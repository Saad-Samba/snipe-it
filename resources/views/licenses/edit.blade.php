    ...
    <div class="col-md-7">
        <select class="form-control" name="discipline_id" id="discipline_id">
            ...
            @foreach(($disciplines ?? collect()) as $discipline)
                <option value="{{ $discipline->id }}" {{ old('discipline_id', $item->discipline_id) == $discipline->id ? 'selected' : '' }}>
                    {{ $discipline->name }}
                </option>
            @endforeach
        </select>
        {!! $errors->first('discipline_id', '<span class="alert-msg" aria-hidden="true"><i class="fas fa-times" aria-hidden="true"></i> :message</span>') !!}
